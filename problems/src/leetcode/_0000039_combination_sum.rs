// 0000039. Combination Sum
//
// Given an array of distinct integers candidates and a target integer target, return a list of all unique combinations of candidates where the chosen numbers sum to target.
//
// You may return the combinations in any order.
//
// The same number may be chosen from candidates an unlimited number of times. Two combinations are unique if the of at least one of the chosen numbers is different.
//
// The test cases are generated such that the number of unique combinations that sum up to target is less than 150 combinations for the given input.
//
// Example 1:
//
// Input: candidates = [2,3,6,7], target = 7
// Output: [[2,2,3],[7]]
//
// Explanation:
// 2 and 3 are candidates, and 2 + 2 + 3 = 7. Note that 2 can be used multiple times.
// 7 is a candidate, and 7 = 7.
// These are the only two combinations.
//
// Example 2:
//
// Input: candidates = [2,3,5], target = 8
// Output: [[2,2,2,2],[2,3,3],[3,5]]
//
// Example 3:
//
// Input: candidates = [2], target = 1
// Output: []
//
// Constraints:
//
//     1 <= candidates.length <= 30
//     2 <= candidates[i] <= 40
//     All elements of candidates are distinct.
//     1 <= target <= 40
//
// TODO: READ ONCE MORE
// - 9.1 Backtracking (the algorithm design manual)
// - 10.5 Unordered Partition or Subset Sum (the algorithm design manual)
// - https://www.geeksforgeeks.org/dsa/backtracking-algorithms/
// - https://www.geeksforgeeks.org/dsa/subset-sum-problem/
// - https://www.geeksforgeeks.org/dsa/combinational-sum/
// ---
pub fn combination_sum(candidates: Vec<i32>, target: i32) -> Vec<Vec<i32>> {
    println!("{candidates:?} {target}");
    todo!()
}
// ---

pub fn testcase() {
    let res = combination_sum(vec![2, 3, 6, 7], 7);
    eprintln!("{} {:?}", module_path!(), res);
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_1() {
        let res = combination_sum(vec![2, 3, 6, 7], 7);
        let expected = vec![vec![2, 2, 3], vec![7]];
        assert_eq!(res, expected);
    }

    #[test]
    fn test_2() {
        let res = combination_sum(vec![2, 3, 5], 8);
        let expected = vec![vec![2, 2, 2, 2], vec![2, 3, 3], vec![3, 5]];
        assert_eq!(res, expected);
    }

    #[test]
    fn test_3() {
        let res = combination_sum(vec![2], 1);
        let expected: Vec<Vec<i32>> = vec![];
        assert_eq!(res, expected);
    }
}
